...
class StaffMarksViewModel extends BaseViewModel {
  ...
  navigateToSearchScreen() async {
    if (selectYearValue!.yearName != "-- SELECT --" && selectSectionValue!.text != "-- SELECT --" && selectSubjectValue!.text != "-- SELECT --" && selectExamTypeValue!.text != "-- SELECT --" && selectExamMasterValue!.text != "-- SELECT --") {
      ...
      log(allStudentMarksList!.list!.length.toString());
      if (allStudentMarksList!.output!.statusFlag == 0) {
        showCustomSnackbar(msg: allStudentMarksList!.output!.statusMessage, type: SnackbarType.failure);
      } else {
        _navigationService.navigateTo(Routes.staffAddMarkScreen, arguments: StaffAddMarkScreenArguments(addMarksModel: addMarksList, dataModel: allStudentMarksList, section: selectSectionValue!.value, subject: selectSubjectValue!.value, classId: selectClassValue!.value, exam: selectExamTypeValue!.value, assesment: selectExamMasterValue!.value, yearId: selectYearValue!.yearId));
      }
    } else {
  ...
  AllStudentMarksList? allStudentMarksList;
  ...
  Future<void> getMarksOnPage() async {
    ...
    try {
      ...
      marksOnPageModel = await apiService.marksOnPageDropdown({"LoginName": _neededVariables.loginModel!.fullName, "LoginId": _neededVariables.loginModel!.loginId, "token": _neededVariables.loginModel!.token, "password": _neededVariables.pass, "cmpid": _neededVariables.loginModel!.cmpid, "brcode": _neededVariables.loginModel!.brcode, "Authentication": _neededVariables.loginModel!.authentication, "PersonId": _neededVariables.loginModel!.personId, "CourseID": _neededVariables.loginModel!.courseId});
      notifyListeners();
      setBusy(false);
    } catch (e) {
      setBusy(false);
  ...
  Future<void> getAllMarksStudent() async {
    ...
    try {
      ...
      ApiService apiService = ApiService(dio, baseUrl: url);
      log(_neededVariables.loginModel!.toMap().toString(), name: "data");
      allStudentMarksList = await apiService.getAllMarkStudentsList({
        "LoginName": _neededVariables.loginModel!.fullName,
        "LoginId": _neededVariables.loginModel!.loginId,
        "token": _neededVariables.loginModel!.token,
        "password": _neededVariables.pass,
      ...
      addMarksList = List<AddMarksModel>.from(allStudentMarksList!.list!.map((x) => AddMarksModel.fromMap(x)));
      notifyListeners();
      setBusy(false);
    } catch (e) {
      setBusy(false);
