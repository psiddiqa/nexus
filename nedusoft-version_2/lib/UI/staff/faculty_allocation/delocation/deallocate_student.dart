...
import 'deallocate_student_view_model.dart';

class StudentDeAllocation extends StatelessWidget {
  List<FacultyDeAllocationStudentList>? studentList;
  SectionList? sectionList;
  SubjectList? subjectList;
  Cl? classes;
  Year? year;
  StudentDeAllocation({required this.studentList, required this.sectionList, required this.subjectList, required this.year, required this.classes});
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder.reactive(
        ...
        viewModelBuilder: () => StudentDeAllocationViewModel(),
        builder: (context, model, child) => LoadingOverlay(
              ...
              child: Scaffold(
                  ...
                  appBar: PreferredSize(preferredSize: Size(MediaQuery.sizeOf(context).width, 60), child: CommonAppBar("Student De-Allocation")),
                                  ...
                                  children: [
                                    SizedBox(
                                        ...
                                        child: SelectWidget(
                                          model: model,
                                          index: index,
                                          studentId: model.studentList![index].attId,
                                        )),
                                    SizedBox(
...
class SelectWidget extends StatefulWidget {
  StudentDeAllocationViewModel model;
  int index;
  int studentId;
  SelectWidget({
    required this.model,
        ...
        if (value!) {
          widget.model.addStudent(
              SaveFacultyDeAllocation(
                attId: widget.studentId,
              ),
              value,
              widget.index);
        } else {
          widget.model.subStudent(SaveFacultyDeAllocation(attId: widget.studentId), value, widget.index);
        }
      },
