...
class EditAttFormViewModel extends BaseViewModel {
  ...
  updateFuction() async {
    ...
    if (getAllStudentAtteListModel!.output!.statusFlag == 0) {
      ...
      _navigationService.navigateTo(Routes.editAttShowScreen, arguments: EditAttShowScreenArguments(to: toTime, from: fromTime, dataModel: getAllStudentAtteListModel, date: dateFormatInYearMonthDate(dateField.text), sectionList: selectSectionValue, subjectList: selectSubjectValue, year: selectYearValue, session: selectSessionValue, saveStudentAttendanceModel: saveStudentAttendanceModel));
    }
  }

  navigateToSearchScreen() async {
    if (_neededVariables.loginModel!.isSubject!) {
      log(selectSubjectValue?.value.toString() ?? "");
  ...
  Future<void> getAllStudentAttList() async {
    ...
    try {
      ...
      ApiService apiService = ApiService(dio, baseUrl: url);
      log(_neededVariables.loginModel!.toMap().toString(), name: "data");
      final result = await apiService.getAllAttendanceList({
        "LoginName": _neededVariables.loginModel?.fullName ?? "",
        "LoginId": _neededVariables.loginModel?.loginId ?? "",
        "token": _neededVariables.loginModel?.token ?? "",
        "password": _neededVariables.pass,
        ...
        "from_time": fromTime == null ? null : timeConverter(fromTime!),
        "to_time": toTime == null ? null : timeConverter(toTime!),
        "Authentication": _neededVariables.loginModel?.authentication,
      });
      getAllStudentAtteListModel = result;
      saveStudentAttendanceModel = List<SaveAttendanceModel>.from(result.list!.map((x) => SaveAttendanceModel.fromMap(x)));
