...
import 'package:nedusoft/models/staff/edit_lesson_update.dart';
...
class StaffEditLessonViewModel extends BaseViewModel {
  ...
  EditLessonUpdateModel? getEnterAttendanceDropdownsModel;
  ...
  init(int id) async {
    ...
    if (getEnterAttendanceDropdownsModel != null) {
      ...
      classList.addAll(getEnterAttendanceDropdownsModel!.classList!);
      yearList.addAll(getEnterAttendanceDropdownsModel!.year!.yearList!);
      sessionList.addAll(getEnterAttendanceDropdownsModel!.sessionList!.list!);
      sectionList.addAll(getEnterAttendanceDropdownsModel!.sectionList!);
      subjectList.addAll(getEnterAttendanceDropdownsModel!.subList!);
      isCombined = getEnterAttendanceDropdownsModel?.isCombined ?? false;
      topic.text = getEnterAttendanceDropdownsModel?.topic ?? "";
      notes.text = getEnterAttendanceDropdownsModel?.notes ?? "";
      dateField.text = DateFormat("dd-MM-yyyy").format(getEnterAttendanceDropdownsModel?.lessonDate ?? DateTime.now());

      for (var val = 0; val < classList.length; val++) {
        if (classList[val].value == getEnterAttendanceDropdownsModel!.classId) {
          log(val.toString());
      ...
      for (var val in sessionList) {
        if (val.value == getEnterAttendanceDropdownsModel!.sessionId) {
          selectSessionValue = val;
        }
      }
      for (var val in yearList) {
        if (val.yearId == getEnterAttendanceDropdownsModel!.yearId) {
          selectYearValue = val;
        }
      }
      for (var val in sectionList) {
        if (val.value == getEnterAttendanceDropdownsModel!.sectionId) {
          selectSectionValue = val;
        }
      }
      for (var val in subjectList) {
        if (val.value == getEnterAttendanceDropdownsModel!.subjectId) {
          selectSubjectValue = val;
        }
      }
    }
  ...
  Future<void> getDropDownResult(int id) async {
    setBusy(true);
    try {
      ...
      final result = await apiService.editStaffLessonUpdate({"LoginName": _neededVariables.loginModel!.fullName, "LoginId": _neededVariables.loginModel!.loginId, "token": _neededVariables.loginModel!.token, "password": _neededVariables.pass, "cmpid": _neededVariables.loginModel!.cmpid, "brcode": _neededVariables.loginModel!.brcode, "Authentication": _neededVariables.loginModel!.authentication, "PersonId": _neededVariables.loginModel!.personId, "CourseID": _neededVariables.loginModel!.courseId, "Value": id});
  ...
  Future<void> addLessonUpdate() async {
    ...
    try {
      ...
      ApiService apiService = ApiService(dio, baseUrl: url);
      log(_neededVariables.loginModel!.toMap().toString(), name: "data");
      final result = await apiService.updateStaffLessonUpdate({
        "LoginName": _neededVariables.loginModel!.fullName,
        "LoginId": _neededVariables.loginModel!.loginId,
        "token": _neededVariables.loginModel!.token,
        "password": _neededVariables.pass,
        ...
        "Topic": topic.text,
        "Notes": notes.text,
        "TopicId": getEnterAttendanceDropdownsModel?.topicId,
      });
      showCustomSnackbar(msg: " Lesson-update Update Successfully", type: SnackbarType.success);
      _navigationService.back();
